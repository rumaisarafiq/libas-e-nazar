const PILLARS = [
  ...
  },
  {
    ...
    desc: 'Key body-joint detection aligns the garment to the model\'s exact posture, so the fit looks accurate from any angle.',
  },
]
  ...
  return (
    <div className="mx-auto max-w-5xl px-6 pb-24 pt-32 lg:px-10 lg:pt-40">
      <div className="text-center">
        <p className="text-xs font-semibold tracking-widest2 text-gold">ABOUT THE PROJECT</p>
        ...
        <p className="mx-auto mt-4 max-w-2xl text-sm leading-relaxed text-charcoal/60 dark:text-cream/60">
          Libas-e-Nazar is a Final Year Project that lets men preview Eastern and
          Western garments on a virtual model before committing to a purchase —
          removing the guesswork from online shopping and reducing costly returns.
        </p>
      </div>
      ...
      <div className="mt-16 grid grid-cols-1 gap-6 sm:grid-cols-2">
        {PILLARS.map((p) => (
          ...
          >
            <h3 className="font-display text-xl text-charcoal dark:text-cream">{p.title}</h3>
      ...
      <div className="mt-16 rounded-2xl bg-white p-8 shadow-soft dark:bg-white/[0.03] sm:p-10">
        <h2 className="font-display text-2xl text-charcoal dark:text-cream">Project scope</h2>
        <ul className="mt-6 space-y-4 text-sm leading-relaxed text-charcoal/70 dark:text-cream/70">
          <li className="flex gap-3">
            <span className="mt-1 h-1.5 w-1.5 shrink-0 rounded-full bg-gold" />
            Frontend built with React, Vite and Tailwind CSS for a responsive, premium
            shopping experience.
          </li>
          <li className="flex gap-3">
            <span className="mt-1 h-1.5 w-1.5 shrink-0 rounded-full bg-gold" />
            Backend planned with FastAPI, serving the trained try-on model and garment
            catalogue through REST APIs.
          </li>
          <li className="flex gap-3">
            <span className="mt-1 h-1.5 w-1.5 shrink-0 rounded-full bg-gold" />
            Covers both Eastern menswear (kurta, sherwani, waistcoat) and Western
            menswear (shirts, polos, pants).
          </li>
        </ul>
      </div>
    </div>
